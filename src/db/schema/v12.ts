...
export class Schema implements IDbSchema {
    public description = "create kv stores for bot sdk";

    public async run(store: DiscordStore): Promise<void> {
        try {
            await store.createTable(
                "CREATE TABLE bsdk_kv (user_id TEXT NOT NULL, key TEXT NOT NULL, value TEXT NOT NULL, PRIMARY KEY (user_id, key));",
                "bsdk_kv",
            );
        } catch (ex) {
